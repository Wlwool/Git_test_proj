...
def find_lowest_cost_node(costs):
    ...
    for node in costs: # перебираем все узлы
        cost = costs[node]
        if cost < lowest_cost and node not in processed:    # если стоимость узла меньше, чем текущая
...
node = find_lowest_cost_node(costs)  # Поиск узла с наименьшей стоимостью среди обработанных
