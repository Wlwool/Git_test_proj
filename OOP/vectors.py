...
# 1. 400 - ошибка запроса, сервер не будет и не может обработать запрос ,
# 401 - ошибка авторизации,
# 429 - слишком много запросов за короткий промежуток времени,
# 503 - сервис недоступен
# 2. Ошибка 429 слишком много запросов показывет,
# что юзер отправил слишком много запросов в одно время
# The 503 error indicates that server is unavailable, reason is overloaded
#
# Урок 2: Docker + технический перевод
# 1. bridge сетевой драйвер по умолчанию, overlay сети объединяют нескольно докер
# демонов вместе, host удаляет изоляцию сети между контейнером и докер хостом
...
#
# Урок 3:
# 1. parser - извлекает данные, tag - метка это такой объект который соответствует
# меткам XML или HTML в оригинальном документе,
# navigable string - используется для хранения фрагментов текста
